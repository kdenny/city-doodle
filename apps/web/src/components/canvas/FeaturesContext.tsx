...
export function FeaturesProvider({
  ...
  useEffect(() => {
    ...
    // CITY-441/CITY-442: Regenerate campus paths for loaded POIs
    const loadedCampusPaths: Road[] = [];
    for (const poi of loadedPOIs) {
      if (poi.footprint && poi.footprint.length >= 3) {
        const paths = generateCampusPaths(poi.footprint, poi.position, poi.type, poi.id);
        loadedCampusPaths.push(...paths);
      }
    }
    ...
    setFeaturesState((prev) => {
      ...
      return {
        ...
        pois: loadedPOIs,
        roads: [...mergedRoads, ...loadedCampusPaths, ...pendingRoads],
      };
    });
  ...
  useEffect(() => {
    ...
    const refreshedPOIs = apiPOIs.map(fromApiPOI);

    // CITY-441/CITY-442: Regenerate campus paths for refreshed POIs
    const refreshedCampusPaths: Road[] = [];
    for (const poi of refreshedPOIs) {
      if (poi.footprint && poi.footprint.length >= 3) {
        refreshedCampusPaths.push(...generateCampusPaths(poi.footprint, poi.position, poi.type, poi.id));
      }
    }
    ...
    setFeaturesState((prev) => ({
      ...prev,
      pois: refreshedPOIs,
      // Replace old campus paths with regenerated ones (campus path IDs start with "poi-" prefix)
      roads: [
        ...prev.roads.filter((r) => !(r.id.startsWith("poi-") && r.id.includes("-path-"))),
        ...refreshedCampusPaths,
      ],
    }));
  }, [worldId, apiPOIs]);
  ...
  const updatePOI = useCallback(
    (id: string, updates: Partial<Omit<POI, "id">>) => {
      ...
      if (effectiveUpdates.footprint && effectiveUpdates.footprint.length >= 3) {
        const newType = updates.type ?? currentPOI.type;
        const newPosition = updates.position ?? currentPOI.position;
        newCampusPaths = generateCampusPaths(effectiveUpdates.footprint, newPosition, newType, id);
      }
