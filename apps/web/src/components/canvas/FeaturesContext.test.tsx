...
describe("FeaturesContext", () => {
  ...
  describe("clearFeatures", () => {
    it("removes all features", async () => {
      ...
      // CITY-235: addDistrict may auto-generate POIs (CITY-345), so capture the count
      const autoGeneratedPoiCount = features!.pois.length;
      ...
      expect(features!.districts.length).toBe(1);
      expect(features!.pois.length).toBe(autoGeneratedPoiCount + 1);
