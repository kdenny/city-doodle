...
class Tile(BaseModel):
    ...
    terrain_status: str = Field(
        default="pending",
        description="Terrain generation status: pending, generating, ready, or failed",
    )
    terrain_error: str | None = Field(
        default=None,
        description="Error message if terrain generation failed",
    )
    created_at: datetime
    updated_at: datetime
